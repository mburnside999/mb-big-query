...
const sqlQuery3 =
  "SELECT title, sum(views) v FROM `bigquery-public-data.wikipedia.pageviews_2022` WHERE DATE(datehour) = '2022-02-07' title not in ('-','Main_Page','Special:Search') and wiki like 'en%' group by title order  by v desc LIMIT 20 ";
const options3 = {
  query: sqlQuery3,
  timeoutMs: 10000, // Time out after 10 seconds.
  useLegacySql: false, // Use standard SQL syntax for queries.
